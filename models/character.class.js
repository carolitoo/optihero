class Character extends MoveableObject {
   ...
   * It also checks if the character is dead or frozen, in which case it does not move.
   */
  moveCharacter() {
    if (this.isDead() || this.characterFrozen) {
      return;
    }
    ...
    if (
      ...
      !this.isAboveGround(this.groundLevelCharacter)
    ) {
  ...
  /**
   * This function handles the death of the character by calling the function to play the death animation.
   * After the animation is played, it displays a lose banner and with a short delay the end screen.
   * It also calls the function to stop the game (clear all intervals).
   */
  async handleDeath() {
    await this.playAnimationDeadCharacter();
   ...
   * After finishing the animation it sets the endGame property to true, freezes the character and stops the animation interval.
   */
  async playAnimationDeadCharacter() {
    if (!isMuted) {
      this.CHARACTER_SOUND_HURT.play();
    }
